...
public interface GameMatchRepository
  ...
  @Query(
      "SELECT gm FROM GameMatch gm WHERE (gm.teamA.uuid = :teamUuid OR gm.teamB.uuid = :teamUuid) AND (gm.teamA.uuid = :otherTeamUuid OR gm.teamB.uuid = :otherTeamUuid) AND gm.status NOT IN :statusList AND gm.reason NOT IN :reasonList ORDER BY gm.finishedAt DESC")
  Page<GameMatch> findTeamMatches(
      @Param("teamUuid") UUID teamUuid,
      @Param("otherTeamUuid") UUID otherTeamUuid,
      @Param("statusList") List<MatchStatus> statusList,
      @Param("reasonList") List<MatchReason> reasonList,
      Pageable pageable);
}
