...
public class AdminCompetitionController {
  ...
  private static final Set<String> ALLOWED_SORT_FIELDS =
      Set.of("createdAt", "name", "slug", "isActive", "isWhitelisted");
  ...
  public Page<AdminCompetitionDto> listAll(@ModelAttribute RestPageRequest pageRequest) {
    ...
    return new PageImpl<>(
        data, competitionsPage.getPageable(), competitionsPage.getTotalElements());
  }
  ...
  public AdminCompetitionDto updateCompetition(
      @PathVariable Long id, @Valid @RequestBody AdminUpdateCompetitionDto input) {
    Competition competition = adminCompetitionService.updateCompetition(id, input);
    return AdminCompetitionDto.from(competition);
  }

  @GetMapping("/{id}")
  @Operation(operationId = "adminGetCompetition", summary = "REST endpoint to get a competition")
