 ...
 *
 * <p>Primary use cases: - Find which TournamentMatch (series) a GameMatch belongs to - List all
 * games in a series for display/tracking
 */
@Repository
public interface TournamentGameRepository extends JpaRepository<TournamentGame, Long> {
  /**
   * Looks up the TournamentGame by the underlying GameMatch. This is the primary entry point when a
   * GameMatch result arrives and we need to find the parent series (TournamentMatch).
   */
  Optional<TournamentGame> findByGameMatch(GameMatch gameMatch);

  /**
   * Returns all games in a series, ordered by game number. Used for DTO construction to show series
   * progress.
   */
