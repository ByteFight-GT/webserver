 ...
 *
 * <p>Path: 1. GameMatchResultHandler detects MatchReason.tournament on a completed GameMatch 2.
 * Calls handleTournamentResult(gameMatch, status) 3. This handler finds the parent TournamentMatch
 * (series) via TournamentGame 4. Updates series win counters based on the game result 5. Decision
 * point: a) DRAW -> Queue another game (draws don't count toward series score) b) Series NOT
 * decided -> Queue the next game (unless 10-game cap reached) c) Series DECIDED -> Mark series
 * complete, update losses/elimination, advance bracket
 *
 * <p>Series rules: - Bo5: first to 3 game wins takes the series - Bo7: first to 4 game wins takes
 * the series (grand finals only) - Draws queue another game without changing scores - A series is
 * force-resolved after 10 total games: most wins wins; if tied, the higher seed advances - A series
 * loss counts as ONE bracket loss for the loser (in double-elimination, two bracket losses =
 * elimination)
 */
@Service
...
public class TournamentResultHandler {
   ...
   *
   * <p>This is the main entry point called by GameMatchResultHandler when a GameMatch with
   * MatchReason.tournament completes.
   *
  ...
  public void handleTournamentResult(GameMatch gameMatch, MatchStatus status) {
    ...
    int gamesPlayed =
        tournamentGameRepository.findByTournamentMatchOrderByGameNumberAsc(match).size();
  ...
  private void completeSeriesByScore(
      TournamentMatch match, TournamentEntry teamOne, TournamentEntry teamTwo) {
    TournamentEntry winner =
        match.getTeamOneSeriesWins() >= match.getWinsRequired() ? teamOne : teamTwo;
    TournamentEntry loser = winner.equals(teamOne) ? teamTwo : teamOne;
    completeSeries(match, winner, loser);
  }

  private void completeSeriesWithCapTiebreak(
      TournamentMatch match, TournamentEntry teamOne, TournamentEntry teamTwo) {
    TournamentEntry winner;
  ...
  private void completeSeries(
    ...
    TournamentEntry loserToAdvance =
        (loser.getStatus() == TournamentEntryStatus.ELIMINATED || !allowLoserAdvance)
            ? null
            : loser;
  ...
  /**
   * Determines whether the loser should advance to the next bracket node. In the grand final, the
   * winners-bracket champion (teamOneEntry by convention) gets one extra life — if they lose, a
   * grand-final reset is scheduled.
   */
  ...
  /**
   * Special-case logic for grand finals: - If winners-bracket champion wins grand final ->
   * tournament complete - If winners-bracket champion loses grand final -> schedule grand-final
   * reset (Bo7) - Grand-final reset winner -> tournament complete
   */
  private void handleGrandFinal(
      TournamentMatch match, TournamentEntry winner, TournamentEntry loser) {
    if (match.getBracketType() == TournamentBracketType.GRAND_FINAL) {
  ...
  private void completeTournament(
      Tournament tournament, TournamentEntry champion, TournamentEntry runnerUp) {
