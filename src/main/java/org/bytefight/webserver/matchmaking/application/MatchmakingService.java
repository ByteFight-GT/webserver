...
public class MatchmakingService {
  ...
  public MatchmakingEvent createAndScheduleEvent(Competition competition, String ladder) {
    ...
    List<TeamStats> teamStats =
        teamStatsRepository.findAllByCompetitionAndLadderAndTeamIn(
            competition, ladder, playableTeams);
        ...
        playableTeams.stream()
            .map(
                team -> {
                  ...
                  if (stats == null) {
                    return Pair.of(
                        team, teamStatsService.getTeamStatsCreateIfNotExist(team, ladder));
                  } else {
  ...
  private List<GameMatch> generateMatches(
      MatchmakingEvent event, String ladder, List<Pair<Team, TeamStats>> participants) {
    List<Pair<Team, TeamStats>> sortedParticipants = new ArrayList<>(participants);
    sortedParticipants.sort(
        Comparator.comparingDouble(
            teamTeamStatsPair -> -teamTeamStatsPair.getRight().getGlickoRating()));
