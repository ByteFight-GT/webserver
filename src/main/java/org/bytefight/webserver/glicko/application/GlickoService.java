...
public class GlickoService {
  ...
  public void processGameMatchResult(GameMatch gameMatch, boolean recalculate) {
    ...
    if (gameMatch.getStatus() != MatchStatus.draw
        && gameMatch.getStatus() != MatchStatus.team_a_win
        && gameMatch.getStatus() != MatchStatus.team_b_win
        && !recalculate) {
    ...
    Ladder ladder =
        ladderRepository
            .findByCompetitionAndLadder(gameMatch.getCompetition(), gameMatch.getLadder())
            .orElseThrow(
                () ->
    ...
    TeamStats teamAStats =
        teamStatsService.getTeamStatsCreateIfNotExist(teamA, gameMatch.getLadder());
    TeamStats teamBStats =
        teamStatsService.getTeamStatsCreateIfNotExist(teamB, gameMatch.getLadder());

    // Don't update stats for matches against self
    if (!teamA.equals(teamB)) {
      GlickoMatchUpdate glickoMatchUpdate =
          calculateMatchUpdate(teamAStats, teamBStats, ladder, gameMatch.getStatus());
      applyGlickoMatchUpdate(glickoMatchUpdate, teamAStats, teamBStats);
    }
  }

  public void applyGlickoMatchUpdate(
      GlickoMatchUpdate glickoMatchUpdate, TeamStats teamAStats, TeamStats teamBStats) {
    ...
    teamAStats.setMatchesPlayed(
        teamAStats.getMatchesPlayed() + teamAUpdate.getMatchesPlayedDelta());
    ...
    teamBStats.setMatchesPlayed(
        teamBStats.getMatchesPlayed() + teamBUpdate.getMatchesPlayedDelta());
  ...
  public GlickoMatchUpdate calculateMatchUpdate(
      TeamStats teamAStats, TeamStats teamBStats, Ladder ladder, MatchStatus matchStatus) {
    double scoreA;
    double scoreB;
    ...
    TeamUpdateResult updatedA =
        calculateUpdatedRating(
