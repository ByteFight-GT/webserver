...
public class AdminLadderController {
  ...
  public Page<AdminLadderDto> listLadders(@ModelAttribute RestPageRequest pageRequest) {
  ...
  @PostMapping
  @Operation(operationId = "adminCreateLadder", summary = "REST endpoint to create a ladder")
  ...
  @PatchMapping("/{id}")
  @Operation(operationId = "adminUpdateLadder", summary = "REST endpoint to update a ladder")
  public AdminLadderDto updateLadder(
      @PathVariable Long id, @Valid @RequestBody AdminUpdateLadderDto input) {
    Ladder ladder = adminLadderService.updateLadder(id, input);
    return AdminLadderDto.from(ladder);
  }

  @GetMapping("/{id}")
  @Operation(operationId = "adminGetLadder", summary = "REST endpoint to get a ladder")
