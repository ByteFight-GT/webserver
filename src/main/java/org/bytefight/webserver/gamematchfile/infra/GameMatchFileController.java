...
@Tag(name = "Game Match File", description = "Endpoints for uploading and retrieving game match result files")
@RestController
...
public class GameMatchFileController {
    ...
    public ResponseEntity<GameMatchFileDto> uploadGameMatchFile(
        ...
        GameMatch gameMatch = gameMatchService.getGameMatch(UUID.fromString(uploadDto.getGameMatchUuid())).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
        Team team = uploadDto.getTeamUuid() != null ? teamService.getTeamByUuid(UUID.fromString(uploadDto.getTeamUuid())).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND)) : null;
...
//                    .body(e.getMessage() + " - " + java.util.Arrays.toString(e.getStackTrace()));
    ...
    @Operation(
            operationId = "getTeamGameMatchFile",
            summary = "Get a team-specific game match file"
    )
    public ResponseEntity<GameMatchFileDto> getTeamGameMatchFile(
        ...
        GameMatchFile gameMatchFile = gameMatchFileService.getByGameMatchAndSlugAndTeam(matchUuid, slug, team).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
        DownloadLinkDto downloadLinkDto = gameMatchFileService.getDownloadLink(user, gameMatchFile);
        return ResponseEntity.ok(GameMatchFileDto.from(gameMatchFile, downloadLinkDto));
    }
}
