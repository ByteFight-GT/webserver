...
class PrivateTeamControllerIT extends FullStackIntegrationTestBase {
  ...
  void editTeamUpdatesTeamSettings() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/v1/team/{uuid}", team.getUuid())
                .with(user(user))
  ...
  void editTeamRejectsNonMember() throws Exception {
    ...
    TeamSettingsDto dto =
        TeamSettingsDto.builder().name("Updated Name").displayMembers(true).build();
  ...
  void editTeamRejectsDuplicateName() throws Exception {
    ...
    TeamSettingsDto dto = TeamSettingsDto.builder().name("taken name").displayMembers(true).build();
  ...
  void editTeamNotFound() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/v1/team/{uuid}", UUID.randomUUID())
                .with(user(user))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(dto)))
  ...
  void setCurrentSubmissionUpdatesTeamSubmission() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/v1/team/{uuid}/current-submission", getTeamUuid(team))
                .with(user(user))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(dto)))
        .andExpect(status().isOk());
    ...
    Submission currentSubmission =
        (Submission) ReflectionTestUtils.getField(updated, "currentSubmission");
