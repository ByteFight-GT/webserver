...
/**
 * Validates bracket sizing, wiring, and series length assignment for the double-elimination bracket
 * builder.
 */
public class TournamentBracketBuilderTest extends FullStackIntegrationTestBase {
  ...
  /**
   * Validates bracket sizing expands to the next power of two and that round 1 sizes are derived
   * from that expanded size.
   *
   * <p>Steps: 1) Build a bracket with 10 entries (not a power of two). 2) Assert the tournament
   * bracket size is expanded to 16. 3) Assert winners round 1 has 16/2 matches (8). 4) Assert
   * losers round 1 has 16/4 matches (4).
   *
   * <p>Expected results: - bracketSize == 16 - winners round 1 size == 8 - losers round 1 size == 4
   */
  @Test
  ...
  /**
   * Validates wiring of winners and losers advancement plus grand final wiring using a basic 8-team
   * bracket.
   *
   * <p>Steps: 1) Build an 8-team bracket. 2) Assign deterministic IDs to all matches to mimic
   * persisted entities. 3) Wire winners advancement, losers advancement, winners/losers to grand
   * final, and grand final reset routing. 4) Inspect the first winners match wiring for winner and
   * loser targets. 5) Ensure the grand final has an ID (created in graph).
   *
   * <p>Expected results: - W1M1 winner advances to W2M1 slot 1. - W1M1 loser drops to L1M1 slot 1.
   ...
   *
   * <p>Steps: 1) Build a 4-team bracket. 2) Assert every winners-bracket match has Bo5 series
   * length and 0-0 counters. 3) Assert every losers-bracket match has Bo5 series length. 4) Assert
   * grand final and grand final reset are Bo7 and correctly typed.
   *
   * <p>Expected results: - Winners/losers matches are Bo5 and counters are 0. - Grand final and
   * reset are Bo7 and have the correct bracket types.
   */
  @Test
  void assignsCorrectSeriesLengths() {
    ...
    assertEquals(
        ...
        graph.getGrandFinal().getSeriesLength(),
    ...
    assertEquals(
        TournamentBracketBuilder.GRAND_FINAL_SERIES_LENGTH,
        graph.getGrandFinalReset().getSeriesLength(),
        "Grand final reset should be Bo" + TournamentBracketBuilder.GRAND_FINAL_SERIES_LENGTH);
    assertEquals(
        TournamentBracketType.GRAND_FINAL_RESET, graph.getGrandFinalReset().getBracketType());
  }

  /**
   * Verifies every match created by the builder starts with zeroed series wins.
   *
   * <p>Steps: 1) Build an 8-team bracket. 2) Iterate all matches in the graph. 3) Assert both team
   * series win counters are 0 for every match.
   *
   * <p>Expected results: - teamOneSeriesWins == 0 for all matches. - teamTwoSeriesWins == 0 for all
   * matches.
   */
  @Test
   ...
   *
   * <p>Steps: 1) Build a 4-team bracket. 2) Read a winners-bracket match (Bo5) and assert
   * winsRequired is 3. 3) Read the grand final (Bo7) and assert winsRequired is 4.
   *
   * <p>Expected results: - Bo5 requires 3 wins. - Bo7 requires 4 wins.
   */
  @Test
   ...
   *
   * <p>Steps: 1) Build a bracket with seeds {3,1,2} for a size-4 bracket. 2) Validate bracket size
   * is 4. 3) Assert round 1 has two matches. 4) Assert the first match is seed 1 vs null (seed 4
   * bye). 5) Assert the second match is seed 2 vs seed 3.
   *
   * <p>Expected results: - Seed order is [1,4,2,3] and missing seed 4 is null. - Match 1: seed 1 vs
   * null, Match 2: seed 2 vs seed 3.
   */
  @Test
   ...
   *
   * <p>Steps: 1) Build a bracket with one entry. 2) Assert bracket size is 1. 3) Assert winners
   * rounds list contains one round with zero matches. 4) Assert losers rounds are empty. 5) Assert
   * only grand final and reset matches exist.
   *
   * <p>Expected results: - bracketSize == 1 - winnersRounds size == 1 and winnersRounds[0] size ==
   * 0 - losersRounds is empty - allMatches size == 2 (grand final + reset)
   */
  @Test
   ...
   *
   * <p>Steps: 1) Build an 8-team bracket. 2) Assert winners rounds sizes [4,2,1] and losers rounds
   * sizes [2,2,1,1,1]. 3) Assert allMatches count is 16 (14 bracket matches + grand final + reset).
   * 4) Assert every winners match is typed WINNERS and round numbers are 1..N. 5) Assert every
   * losers match is typed LOSERS and round numbers are 1..M.
   *
   * <p>Expected results: - Winners rounds sizes match the expected power-of-two progression. -
   * Losers rounds sizes match the double-elimination structure. - All matches are correctly typed
   * and numbered.
   */
  @Test
   ...
   *
   * <p>Steps: 1) Build a 4-team bracket. 2) Assign deterministic IDs to all matches. 3) Wire
   * winners advancement. 4) Assert W1M1 points to W2M1 slot 1. 5) Assert W1M2 points to W2M1 slot
   * 2. 6) Assert the final winners match has no next-winner target.
   *
   * <p>Expected results: - W1M1 next winner = W2M1 slot 1. - W1M2 next winner = W2M1 slot 2. - W2M1
   * has no next-winner wiring.
   */
  @Test
   ...
   *
   * <p>Steps: 1) Build a 2-team bracket (single winners round). 2) Assign deterministic IDs. 3)
   * Wire winners advancement. 4) Assert the only match has no next-winner target or slot.
   *
   * <p>Expected results: - nextWinnerMatchId and nextWinnerSlot remain null.
   */
  @Test
   ...
   *
   * <p>Steps: 1) Build a 2-team bracket. 2) Assign deterministic IDs. 3) Call wireLosersAdvancement
   * with an empty losers rounds list. 4) Assert winners match has no loser routing set.
   *
   * <p>Expected results: - nextLoserMatchId and nextLoserSlot remain null.
   */
  @Test
   ...
   *
   * <p>Steps: 1) Build an 8-team bracket and assign deterministic IDs. 2) Wire losers advancement.
   * 3) Assert W1 losers drop into L1 with correct slots (1,2). 4) Assert W2 losers drop into L2
   * with slot 2. 5) Assert W3 loser drops into L4 with slot 2. 6) Assert losers rounds advance: -
   * Odd-indexed transitions keep 1:1 mapping into slot 1. - Even-indexed transitions merge into the
   * next round with slots 1 and 2.
   *
   * <p>Expected results: - Specific nextLoserMatchId/nextLoserSlot mappings from winners rounds. -
   ...
   *
   * <p>Steps: 1) Build a 2-team bracket (no losers rounds). 2) Assign deterministic IDs. 3) Wire
   * losers-to-grand-final with an empty losers list. 4) Assert winners final points to grand final
   * slot 1.
   *
   * <p>Expected results: - Winners final has nextWinnerMatchId set to grand final and slot 1.
   */
  @Test
  void wireLosersToGrandFinalHandlesEmptyLosersRounds() {
    ...
    tournamentBracketBuilder.wireLosersToGrandFinal(
        graph.getWinnersRounds(), List.of(), graph.getGrandFinal());
   ...
   *
   * <p>Steps: 1) Build a 4-team bracket. 2) Assign deterministic IDs. 3) Wire
   * losers-to-grand-final. 4) Assert winners final goes to grand final slot 1. 5) Assert losers
   * final goes to grand final slot 2.
   *
   * <p>Expected results: - Winners champion feeds slot 1, losers champion feeds slot 2.
   */
  @Test
  void wireLosersToGrandFinalSetsBothChampions() {
    ...
    TournamentMatch losersFinal =
        graph.getLosersRounds().get(graph.getLosersRounds().size() - 1).get(0);
   ...
   *
   * <p>Steps: 1) Create two independent grand final/reset pairs. 2) Assign IDs to each pair. 3)
   * Wire grand final reset for each pair. 4) Assert each grand final points to its reset with slot
   * 1.
   *
   * <p>Expected results: - Each grand final has nextLoserMatchId set to its reset and slot 1.
   */
  @Test
   ...
   *
   * <p>Steps: 1) Invoke createMatch via reflection for a WINNERS match with both teams. 2) Invoke
   * createMatch via reflection for a GRAND_FINAL match with null teams. 3) Assert bracket type,
   * round, index, and default state. 4) Assert series length is Bo5 for winners, Bo7 for grand
   * final. 5) Assert series win counters start at 0 and teams are set correctly.
   *
   * <p>Expected results: - Winners match is Bo5, PENDING, 0-0, and has both teams. - Grand final
   * match is Bo7 and typed GRAND_FINAL.
   */
  @Test
  void createMatchAssignsSeriesLengthAndDefaults() {
    ...
    TournamentMatch winnersMatch =
        invokeCreateMatch(
            ...
            teamOne,
            teamTwo);
    TournamentMatch grandFinalMatch =
        invokeCreateMatch(
            tournamentBracketBuilder,
            tournament,
    ...
    assertAll(
        ...
        () ->
            assertEquals(
                TournamentBracketBuilder.NORMAL_SERIES_LENGTH, winnersMatch.getSeriesLength()),
        ...
        () ->
            assertEquals(
                TournamentBracketBuilder.GRAND_FINAL_SERIES_LENGTH,
                grandFinalMatch.getSeriesLength()),
   ...
   *
   * <p>Steps: 1) Invoke nextPowerOfTwo via reflection for inputs 0,1,2,3,5. 2) Assert each result
   * is the smallest power of two >= input.
   *
   * <p>Expected results: - 0 -> 1 - 1 -> 1 - 2 -> 2 - 3 -> 4 - 5 -> 8
   */
  @Test
   ...
   *
   * <p>Steps: 1) Invoke generateSeedOrder via reflection for sizes 2, 4, and 8. 2) Assert the
   * generated sequence matches expected seed ordering.
   *
   * <p>Expected results: - Size 2: [1, 2] - Size 4: [1, 4, 2, 3] - Size 8: [1, 8, 4, 5, 2, 7, 3, 6]
   */
  @Test
  ...
  private static List<Integer> invokeGenerateSeedOrder(int size) {
    try {
      Method method =
          TournamentBracketBuilder.class.getDeclaredMethod("generateSeedOrder", int.class);
      method.setAccessible(true);
  ...
  private static TournamentMatch invokeCreateMatch(
      TournamentBracketBuilder builder,
    ...
    try {
      Method method =
          TournamentBracketBuilder.class.getDeclaredMethod(
              "createMatch",
              Tournament.class,
      ...
      return (TournamentMatch)
          method.invoke(builder, tournament, bracketType, round, matchIndex, teamOne, teamTwo);
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
  }
