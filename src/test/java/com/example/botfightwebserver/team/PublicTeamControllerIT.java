...
class PublicTeamControllerIT extends FullStackIntegrationTestBase {
    ...
    void getPublicTeamByUuidReturnsTeamWithoutMembersWhenDisplayMembersFalse() throws Exception {
        ...
        MvcResult result = mockMvc.perform(get("/api/v1/public/team/{uuid}", team.getUuid()))
                .andExpect(status().isOk())
                .andReturn();

        PublicTeamDto dto = objectMapper.readValue(result.getResponse().getContentAsString(), PublicTeamDto.class);
    ...
    void getPublicTeamByUuidNotFound() throws Exception {
        mockMvc.perform(get("/api/v1/public/team/{uuid}", UUID.randomUUID()))
    ...
    void getPublicTeamByUuidDeletedTeamNotFound() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/public/team/{uuid}", team.getUuid()))
                .andExpect(status().isNotFound());
    }
}
